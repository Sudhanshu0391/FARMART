...
<script>
	...
	var product=<?php echo json_encode($profile->ware,JSON_PRETTY_PRINT) ?>;
...
<td><input list="cat" id="myInput1" onkeyup="myFunction()" placeholder="Search for Category.." title="Type in a city">
<datalist id='cat'>
...
<td><input type="text" id="myInput2" onkeyup="myFunction()" placeholder="Search for Warehouse Name.." title="Type in a product"></td>
</tr>
</table>
<center>
...
<?php 
	$I=0;
	foreach($profile->ware as $prod){ ?>
		
		<div class="fast-transition" id="<?php $id=$prod['WARE_ID']; echo $id; ?>" style="height:450px; width:24.2%;float:left;margin:5px 5px 10px 5px;" onclick="page('<?php echo $id;?>')">
		<br>
		<center>
		<img style="height:250px;width:250px" src="img/<?php echo $prod['PHOTO'];?>" alt="<?php echo $prod['PHOTO'];?>" width="250px" height="250px">
		</center>
		<br><br>
		<span style="float:left;padding-left:20px;font-size:20px"><?php echo $prod['WARE_NAME'];?></span>
		<br>
		<div style="float:left">
		<span style="padding-left:20px;text-align:left;float:left;width:100%;color:grey;font-size:15px"><?php $s=strlen( $prod['W_DESC']); if($s>80){echo substr($prod['W_DESC'],0,80)."..."; echo "<br>";}else{$x=$prod['W_DESC'];$len=74-strlen($x);echo $x; for($i=0;$i<$len;$i++){echo "&nbsp;";}  }?></span>
		</div>
		<br><br>
		<span style="float:left;padding-left:20px;font-size:15px"><b>&#8377;<?php echo $prod['PRICE'];?></b>/Month</span><br>
		<span style="float:left;padding-left:20px;font-size:15px">Area:<?php echo $prod['AREA'];?> sq.ft</span>
...
function myFunction() {
  ...
  if(input.value.length!=0 && input1.value.length!=0 && input2.value.length!=0){
	  for (i = 0; i < product.length; i++) {
		city = product[i].CITY;
		domain=product[i].CATEGORY;
		name=product[i].WARE_NAME;
		  if (city.toUpperCase().indexOf(filter) > -1 && domain.toUpperCase().indexOf(filter1) > -1 && name.toUpperCase().indexOf(filter2) > -1) {
			document.getElementById(product[i].WARE_ID).style.display = "";
		  } else {
			document.getElementById(product[i].WARE_ID).style.display ="none";
		  }      
	  }
  }
  else if(input.value.length!=0 && input1.value.length!=0){
	  for (i = 0; i < product.length; i++) {
		city = product[i].CITY;
		domain=product[i].CATEGORY;
		name=product[i].WARE_NAME;
		  if (city.toUpperCase().indexOf(filter) > -1 && domain.toUpperCase().indexOf(filter1) > -1) {
			document.getElementById(product[i].WARE_ID).style.display = "";
		  } else {
			document.getElementById(product[i].WARE_ID).style.display ="none";
		  }      
	   ...
	   for (i = 0; i < product.length; i++) {
		...
		domain=product[i].CATEGORY;
		name=product[i].WARE_NAME;
		  if ( domain.toUpperCase().indexOf(filter1) > -1 && name.toUpperCase().indexOf(filter2) > -1) {
			document.getElementById(product[i].WARE_ID).style.display = "";
		  } else {
			document.getElementById(product[i].WARE_ID).style.display ="none";
		  }      
    ...
    else if(input.value.length!=0 && input2.value.length!=0){
	   for (i = 0; i < product.length; i++) {
		city = product[i].CITY;
		domain=product[i].CATEGORY;
		name=product[i].WARE_NAME;
	   ...
	   for (i = 0; i < product.length; i++) {
		...
		domain=product[i].CATEGORY;
		name=product[i].WARE_NAME;
		  if ( domain.toUpperCase().indexOf(filter1) > -1) {
			document.getElementById(product[i].WARE_ID).style.display = "";
		  } else {
			document.getElementById(product[i].WARE_ID).style.display ="none";
		  }      
